...
export default function InternGrid({ programs, categories }: { programs: P[]; categories: string[] }) {
  const [sel, setSel] = useState<string[]>([])
  const [q, setQ] = useState('')
  const toggle = (c: string) => setSel((s) => (s.includes(c) ? s.filter((x) => x !== c) : [...s, c]))
  const filtered = programs.filter(
    (p) => (sel.length === 0 || sel.includes(p.category)) && (q === '' || (p.title + ' ' + p.field).toLowerCase().includes(q.toLowerCase()))
  )
  return (
    <div className="wrap dwrap" style={{ display: 'grid', gridTemplateColumns: '262px 1fr', gap: 30, alignItems: 'start' }}>
      <aside className="dfilter" style={{ position: 'sticky', top: 90, background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 18, padding: '20px 20px 24px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--gold-soft)', color: 'var(--gold)', borderRadius: 999, padding: '6px 14px', fontSize: 13, fontWeight: 600, marginBottom: 18 }}>🇬🇧 Londra · UCL</div>
        <div className="fl">Alan ara</div>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ör. Tıp, Hukuk, Yazılım…" className="fi" />
        <div className="fl" style={{ marginTop: 18 }}>Kategoriler</div>
        <div style={{ display: 'grid', gap: 4, marginTop: 4 }}>
          {categories.map((c) => {
            const on = sel.includes(c)
            const n = programs.filter((p) => p.category === c).length
            return (
        ...
        <div style={{ marginTop: 22, padding: '14px 14px', background: 'rgba(8,16,30,.5)', border: '1px solid var(--line)', borderRadius: 12 }}>
          <div style={{ fontSize: 12.5, color: 'var(--mut)', lineHeight: 1.6 }}>Tüm programlar <b style={{ color: 'var(--txt)' }}>15–18 yaş</b>, Londra UCL Kampüsü, 1–2 hafta. <b style={{ color: 'var(--gold)' }}>£2.225</b>'ten başlar.</div>
        </div>
      </aside>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(258px,1fr))', gap: 20 }}>
          {filtered.map((p) => (
            <Link key={p.slug} href={`/lise-staj/${p.slug}`} className="scard">
              <div className="ph">
                <img className="bg" src={p.photo} alt="Londra" loading="lazy" />
                <span className="pr">£{p.priceFrom.toLocaleString('tr-TR')}'ten</span>
                <span className="ct">{p.category}</span>
                <div className="ov">
                  <h4>{p.title}</h4>
                  <div className="loc">{p.ageMin}–{p.ageMax} yaş · Londra, UCL</div>
                </div>
              </div>
              <div className="body">{p.dates}</div>
            </Link>
          ))}
        </div>
        {filtered.length === 0 && <p style={{ color: 'var(--mut2)', padding: '40px 0', textAlign: 'center' }}>Bu filtreyle program bulunamadı.</p>}
      </div>
      ...
      <style>{`
        ...
        .scard .ph .ct{position:absolute;top:10px;left:10px;z-index:2;background:rgba(255,255,255,.92);color:#0a1426;font-size:10.5px;font-weight:700;padding:4px 9px;border-radius:8px}
